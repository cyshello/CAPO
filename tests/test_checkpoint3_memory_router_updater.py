...
def updater_inputs(tmp_path, *, candidate_bank=None, mapping=None, memory_value=None):
    ...
    wrapper_path = write_json(tmp_path / "candidate_codebook.json", {
        "schema_version": "memory_candidate_codebook_v1",
...
def test_updater_receives_candidate_codebook_mapping_and_bounded_evidence(tmp_path):
    ...
    assert request["schema_version"] == "memory_router_updater_request_v1"
...
def test_positive_and_negative_examples_insert_and_unsupported_rejected(tmp_path):
    actions = [
    ...
    assert len(row["positive_examples"]) == 1
    assert len(row["negative_examples"]) == 1
    report = loaded(result, "validation_report")
    assert report["accepted_action_ids"] == ["p", "n"]
    assert "supported positive effect" in report["rejected_actions"][0][
        "reasons"][0]
    assert "conflicting polarity" in report["rejected_actions"][1]["reasons"][0]


def test_stale_id_and_downstream_leakage_are_rejected(tmp_path):
    ...
    assert any("downstream QA" in reason for reason in reasons)
...
def codebook_checkpoint(tmp_path, *, memory_value=None):
    ...
    updater_manifest = write_json(tmp_path / "cp2" / "updater_manifest.json", {
        "schema_version": "memory_codebook_checkpoint_manifest_v1",
