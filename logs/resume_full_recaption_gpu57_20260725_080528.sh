...
# Both arms of the scope comparison run the generator at minimal (confirmed
# 2026-07-26 for the meta-prompt run on the other host), so this run stays
# minimal end to end. Stated explicitly because the stack file is the only
# other place it is set and nothing in the run artifacts records it.
export SR_GENERATOR_REASONING_EFFORT=minimal
...
export PROMPT_DELTA_WORKER_GPUS=0,1,2,4,5
...
export PROMPT_DELTA_ITERATION_TIMESTAMP=20260725_080528   # resume key: same root, inputs, caches
