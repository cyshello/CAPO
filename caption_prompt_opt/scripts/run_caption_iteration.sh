...
WORKER_GPUS="${FRESH_PROMPT_DELTA_WORKER_GPUS:-0,1,2,3}"
...
# updater, Qwen captioner, DVD tool); they are no-ops until these env vars arm
# them. The EXIT trap sums the per-iteration ledger and prints a token report.
# Set SR_TOKEN_LEDGER_DIR yourself to override, or unset all three to skip.
...
# Optimizer (feedback/proposer/updater) stack — reused from the incumbent stack.
OPTIMIZER_MODEL_ID="${FRESH_PROMPT_DELTA_OPTIMIZER_MODEL_ID:-gpt-5-mini}"
OPTIMIZER_MAX_OUTPUT_TOKENS="${FRESH_PROMPT_DELTA_OPTIMIZER_MAX_OUTPUT_TOKENS:-32000}"
if [[ ! "$OPTIMIZER_MAX_OUTPUT_TOKENS" =~ ^[1-9][0-9]*$ ]]; then
  echo "optimizer output token budget must be a positive integer" >&2
  exit 2
fi
# Caption updater policy version (the caption-prompt updater prompt is applied
# in caption_prompt_opt.factory, not by this name; kept distinct for provenance).
