...
``PromptDeltaInterventionRunner`` -> ``FullRecaptionInterventionRunner`` on the
evidence entry. The OpenAI free-form prompt generator is left ACTIVE (this is the
meta path, not the static caption path), and the proposer stays localized so its
untruncated request does not overflow — only the recaption apply-scope grows.
