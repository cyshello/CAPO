#!/usr/bin/env bash
# Operator-run only: sequential configurable-K FULL-RECAPTION iterations over
# explicit frozen evidence and confirmation cohorts. Copy of
# caption_prompt_opt/scripts/run_caption_kiter.sh with the full-recaption swaps:
#   * per-iteration driver -> full_recaption_opt/scripts/run_full_recaption_iteration.sh
#   * parent chain starts at the caption prompt, advances by promoted id
#   * run/experiment dirs prefixed full_recaption_
...
EXPERIMENT_LABEL="${PROMPT_DELTA_EXPERIMENT_LABEL:-full_recaption_20video_5iter}"
if [[ ! "$EXPERIMENT_LABEL" =~ ^[A-Za-z0-9_-]+$ ]]; then
  echo "PROMPT_DELTA_EXPERIMENT_LABEL contains unsupported characters" >&2
  exit 2
fi
EXPERIMENT_ROOT="$PROJECT_ROOT/runs/full_recaption_${EXPERIMENT_LABEL}_${EXPERIMENT_TIMESTAMP}"
STATE_ROOT="${EXPERIMENT_ROOT}_state"
CACHE_ROOT="${EXPERIMENT_ROOT}_cache"
MEMORY_ROOT="${EXPERIMENT_ROOT}_feedback_memory"
CURRENT_PARENT="$PROJECT_ROOT/caption_prompt_opt/prompts/init_caption_prompt.json"
...
for ordinal in $(seq 1 "$ITERATION_COUNT"); do
  ...
  FRESH_PROMPT_DELTA_PROMOTION_POLICY="promote_and_enqueue_measurement_v1" \
    bash "$CPO_ROOT/scripts/run_full_recaption_iteration.sh"

  evidence_manifest="$PROJECT_ROOT/runs/full_recaption_iteration_${run_timestamp}_evidence/fresh_evidence_manifest.json"
  status="$(jq -r '.status' "$evidence_manifest")"
  if [[ "$status" != "no_eligible_proposal_evidence" ]]; then
    result="$PROJECT_ROOT/runs/full_recaption_iteration_${run_timestamp}_output/iteration_result.json"
