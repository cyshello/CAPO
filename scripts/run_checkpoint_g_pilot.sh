...
PARENT="${PARENT_META_PROMPT_ARTIFACT:-$PROJECT_ROOT/optimization/prompts/init_meta_prompt.json}"
PARENT_META_PROMPT_ID="$(jq -er '.meta_prompt_id' "$PARENT")"
FEEDBACK_MEMORY_BANK_ROOT="$PROJECT_ROOT/runs/prompt_delta_feedback_memory_bank"
...
if [[ ! -f "$INPUT_ROOT/manifest.json" ]]; then
  ...
  conda run --no-capture-output -n "$SR_CONDA_ENV" \
    ...
    --scaffold-components "$PROJECT_ROOT/prompt_routing/fixtures/static_meta_replace_body_components.json" \
