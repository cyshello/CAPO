...
if [[ ! -f "$INPUT_ROOT/manifest.json" ]]; then
  ...
  conda run --no-capture-output -n "$SR_CONDA_ENV" \
    ...
    --proposer-context-limit "${FRESH_PROMPT_DELTA_PROPOSER_CONTEXT_LIMIT:-128000}" \
    ...
    --feedback-context-limit "${FRESH_PROMPT_DELTA_FEEDBACK_CONTEXT_LIMIT:-128000}" \
    ...
    --updater-context-limit "${FRESH_PROMPT_DELTA_UPDATER_CONTEXT_LIMIT:-128000}" \
